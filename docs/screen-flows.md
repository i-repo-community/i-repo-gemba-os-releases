...
- 差分が大きい場合や過去へ遡って取り込みたい場合は、「**安全取得（時間窓分割）**」（折りたたみを開いて設定）を有効にします。期間を小さな実行単位に分けて順に取得するため、途中で失敗・中止しても済んだ分の取り直しは不要です。件数が多い窓は自動で半分に分け、「取得開始日」を指定すると過去（例: 10年前）からの遡及取得（バックフィル）もできます。
- 安全取得の折りたたみにある「**未変更の帳票は成果物の再取得をスキップ**」を有効にすると、前回の配信から内容（rev）が変わっていない帳票の PDF/Excel の再ダウンロードを省き、通信量とサーバ負荷を下げられます。スキップされた帳票はそのバッチに実ファイルが含まれず、前回バッチへの参照になります（クラスター値は従来どおり取り込まれます）。バックフィルの2回目以降の実行が軽くなります。複数配信先（fan-out）や upload（配置）のフローでは使えません。
