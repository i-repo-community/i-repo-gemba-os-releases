...
安全取得の実行時間帯（フロー作成の「安全取得（時間窓分割）」で設定）の外での OS スケジューラ発火は、失敗ではなくスキップとして 202 を返します（run は作られません）:

```json
{"ok":true,"batchId":null,"trigger":"schedule","skipped":true,"reason":"実行時間帯外のため、スケジュール実行をスキップしました（手動実行は時間帯に関係なく可能です）"}
```
