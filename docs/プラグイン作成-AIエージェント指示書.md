...
# プラグイン作成 — AI エージェント指示書（コピペ用）

このページは**エージェント（Claude Code / Codex CLI 等）に丸ごと渡すための指示書**です。人間が読む必要は
ありません（読み物は **[プラグイン作成ガイド](プラグイン作成ガイド.md)**）。

**使い方**: 下のブロックを**まるごとコピー**（ブロック右上のコピーボタン、または全選択）して、エージェントに貼り、
続けて「**〜へ配信するプラグインを作って**」と伝えてください。例:「社内 PostgreSQL に配信するプラグインを作って」
「受け取った帳票を CSV に追記するプラグインを作って」。可搬なので、そのまま作業リポジトリ
（`i-repo-community/i-repo`）の `CLAUDE.md` に貼っても使えます。

````text
# i-Repo プラグイン作成 — あなた（AI エージェント）への指示
# （この全文をあなたへの指示として扱ってください）

あなたはこれから i-Repo のプラグインを作ります。プラグインとは、i-Reporter の帳票データを外部ストアへ
配信（write→verify）し、必要なら読み返す（query）、1 ファイルの Node 実行ファイル（i-repo-<name>、
1 行目 #!/usr/bin/env node）です。~/.i-repo/plugins/ に置くと i-repo <name> ... として呼ばれます。

大原則: 以下の規約と手順をそのまま守れば動くものが作れます。API を推測で作らない。迷ったら「深掘り資料」の
spec と実プラグインを読んで確認する。最後に必ず自己テストを緑にしてから完了報告する。
...
2. 骨格から始める: 下の「最小スケルトン」をコピーし、i-repo-<name> として保存。name は ^[A-Za-z0-9][A-Za-z0-9_-]*$。
3. 配信（write / verify）を実装:
   ...
   - 書いたものを読み返して照合 → 一致で verified:true の verify receipt。
4. （任意）読み返し（query）を実装: lib/gemba-read.js を使う。行を emitRow で出し、最後に必ず queryEnd
   （＝query-end トレーラ。0 件でも出す）。読み取り専用を厳守（1 バイトも書かない）。
...
7. 完了報告: 作ったファイルのパス、何をするプラグインか、テスト結果（緑）、実行例。

## 絶対規約（外すと「動いてるのに失敗/切断扱い」になる）

- 配信の成功 ＝ receipt の verified:true のみ（exit 0 は成功ではない）。receipt の必須フィールド:
