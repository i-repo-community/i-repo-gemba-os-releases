...
同じ帳票を何度配信しても**重複せず、いつも最新の内容で上書き**されます。
...
mongo は受け取った帳票の一覧（NDJSON）を MongoDB へ upsert（同じ帳票は重複せず最新で上書き）します。
`_id` は冪等キー（`idempotencyKey`、なければ `report:<itemId>:rev<revNo>`）です。レコードを丸ごと
