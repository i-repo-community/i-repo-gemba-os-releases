...
|---|---|---|
| **GCS** | **サービスアカウント鍵（JSON）** | 「サービスアカウント鍵ファイルのパス」に鍵 JSON の置き場所を入れる（`--gcp-credentials`）。鍵は最小権限（対象バケットへの書き込みのみ）で発行する |
...
- **最小権限**で発行する（対象バケット/コンテナへの**書き込みのみ**。読み取り・削除は付けない）。
- **秘密は伏字で保存**され、配信時は**コマンド引数ではなく環境変数**で渡されます（プロセス一覧やログに平文で出ません）。
- **GCS の鍵は「パス」**を渡します（鍵 JSON 本体はアプリに貼り付けず、ファイルとして安全な場所に置く）。
