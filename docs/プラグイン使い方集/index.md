...
下の表で、現在配布している8本の役割と使い方を確認できます。

| プラグイン | 役割 | 対応OS | ドキュメント |
|---|---|---|---|
| **archive** | 帳票（PDF / Excel と項目の入力値）をまとめて保管し、必要なら S3 / GCS / Azure Blob へアップロード | mac / Linux / Win | [i-repo-archive](i-repo-archive.md) |
...
| **upload** | PDF / Excel を送り先へ実名のまま配置（ファイル共有向け） | mac / Linux / Win | [i-repo-upload](i-repo-upload.md) |
| **report-delete** | 配信済みを確認した帳票だけを i-Reporter から安全に削除 | mac / Linux / Win | [i-repo-report-delete](i-repo-report-delete.md) |

> S3 / GCS / Azure Blob への配信は独立プラグインではなく **archive の `push-*`**（帳票をまとめてクラウドへアップロード）で行います。上表の archive を参照。
...
| PDF / Excel の実体ファイルをクラウドに保管したい | **archive**（S3 / GCS / Azure Blob） |
| PDF / Excel をファイル共有先へ分かりやすい名前で置きたい | **upload** |
| まずは帳票をローカルにまとめて保管したい | **archive** |
| 外部へ配信済みの古い帳票を安全に削除したい | **report-delete** |
...
```
        ...
        ▼                        ▼                         ▼
   ┌─────────┐            ┌────────────────────────┐      ┌──────────────────┐
   │ archive │   一覧     │      送り先プラグイン        │      │ archive / upload  │
   │ create  │──データ──▶ │ mongo / sqlite / elastic │      │  （PDF / Excel）   │
   └─────────┘            │ parquet / bigquery       │      └──────────────────┘
                          │    （項目の入力値など）        │       一覧と実体を突合
                          └────────────────────────┘
                       保管 / BI / 検索 / 分析基盤 / DWH
...
- **実体ファイル**（PDF / Excel）は archive で保管、または upload でファイル共有先へ実名配置
- **配信済み帳票の削除**は report-delete が安全条件を再確認して実行
...
- **入力(intake)の種類**: プラグインがデータを受け取る方法です。`archive create` は入力なし（自分で取り出す）、
  ...
  parquet / bigquery）は標準入力の NDJSON（一覧データを1行1帳票で流し込む）を受け取ります。`upload` は
  archive が作った保管フォルダを受け取り、`report-delete` は削除画面の安全確認済み条件を受け取ります。
